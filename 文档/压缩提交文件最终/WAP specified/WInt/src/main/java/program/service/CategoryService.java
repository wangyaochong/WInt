...
public class CategoryService {
    ...
    public Page<Category> queryPage(Category queryExample, Integer pageNum, Integer pageSize, String orderBy, Boolean OrderAsc){
        ...
        if (ObjectUtil.checkAllFieldNullOrEmptyAndSetEmptyNull(queryExample)) {
            if(StringUtils.isNullOrEmpty(orderBy)){
                return categoryRepo.findAll(new PageRequest(pageNum,pageSize));
            }else{
                return categoryRepo.findAll(new PageRequest(pageNum,pageSize,direction,orderBy));
            }
        }
        ...
        if(StringUtils.isNullOrEmpty(orderBy)){//如果不排序
            ...
            return categoryRepo.findAll( Example.of(queryExample,ExampleMatcher.matching()
                            .withStringMatcher(ExampleMatcher.StringMatcher.CONTAINING)
                            .withIgnoreCase()),
