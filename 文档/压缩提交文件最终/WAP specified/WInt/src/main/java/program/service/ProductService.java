...
public class ProductService {
    ...
    public Page<Product> queryPage(Product queryExample, Integer pageNum, Integer pageSize, String orderBy, Boolean OrderAsc){
        ...
        if (ObjectUtil.checkAllFieldNullOrEmptyAndSetEmptyNull(queryExample)) {
            if(StringUtils.isNullOrEmpty(orderBy)){
                return productRepo.findAll(new PageRequest(pageNum,pageSize));
            }else{
                return productRepo.findAll(new PageRequest(pageNum,pageSize,direction,orderBy));
            }
        }
        ...
        if(StringUtils.isNullOrEmpty(orderBy)){//如果不排序
            ...
            return productRepo.findAll( Example.of(queryExample,ExampleMatcher.matching()
                            .withStringMatcher(ExampleMatcher.StringMatcher.CONTAINING)
                            .withIgnoreCase()),
